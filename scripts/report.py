"""Render the dashboard: data/processed/report.html, self-contained.

    python scripts/report.py          # after `vct update`

Everything on the page is computed here and embedded as JSON, so the file opens
anywhere with no server and no database. Re-run it to refresh.
"""
